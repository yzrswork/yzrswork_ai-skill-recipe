...
- 未確認情報を禁止事項として断定しない。
- 潮汐は出典が確定したデータのみ格納する。推測値やスクレイピング由来の不安定な値をSSOTにしない。
...
- `date`: `YYYY-MM-DD`
- `tideName`: 大潮/中潮/小潮/長潮/若潮等。
- `events[]`: `high` / `low`, 時刻、潮位cm。
- 必要ならUI側で現在時刻から上げ/下げ、次イベントまでの残り時間を算出する。
