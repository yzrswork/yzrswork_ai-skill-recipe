...
| `spark-score/` | ひかり譜 / Spark Score v2 | 空を最大16回タップして譜面を作り、位置と間隔を保ったまま、速度・方向・音色・エンベロープ・フィルター・3スロットFX・LFO・4つのアサイナブルマクロ・XYで演奏を変形するiPhone向け可変楽器。MUTATE／CHAOSは譜面を変更せずパッチだけを変え、v1/v2共有URLと最後のパッチを端末内で扱う。音声はブラウザ内生成、APIなし。 |
